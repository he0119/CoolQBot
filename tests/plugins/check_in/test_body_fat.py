...
async def test_target_body_fat(app: App):
    ...
    async with app.test_matcher() as ctx:
        ...
        event = fake_group_message_event_v11(message=Message("/目标体脂"))
        ctx.receive_event(bot, event)
        ctx.should_call_send(event, "你的目标体脂是 55.0%，继续努力哦～", True, at_sender=True)
        ctx.should_finished(target_body_fat_cmd)
    ...
    async with app.test_matcher() as ctx:
        ...
        event = fake_group_message_event_v11(message=Message("/目标体脂"))
        ctx.receive_event(bot, event)
        ctx.should_call_send(event, "你的目标体脂是 50.0%，继续努力哦～", True, at_sender=True)
        ctx.should_finished(target_body_fat_cmd)
...
async def test_body_fat_invalid(app: App):
    ...
    async with app.test_matcher() as ctx:
        ...
        event = fake_group_message_event_v11(message=Message(MessageSegment.at(123456789)))
        ctx.receive_event(bot, event)
        ctx.should_call_send(event, "今天你的体脂是多少呢？", True)
        ctx.should_rejected(body_fat_record_cmd)
    ...
    async with app.test_matcher() as ctx:
        ...
        ctx.receive_event(bot, event)
        ctx.should_call_send(event, "体脂只能输入数字哦，请重新输入", True, at_sender=True)
        ctx.should_rejected(body_fat_record_cmd)
    ...
    async with app.test_matcher() as ctx:
        ...
        ctx.receive_event(bot, event)
        ctx.should_call_send(event, "目标体脂只能在 0% ~ 100% 之间哦，请重新输入", True, at_sender=True)
        ctx.should_rejected(body_fat_record_cmd)
