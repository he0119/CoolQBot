...
- 将纯对话与工具调用分别独立为 `/chat` 和 `/agent`，使 `/llm` 专用于 `model`、`tts` 与 `quota`，并将 TTS 管理统一为 `list` / `set` 子命令；@ 对话复用纯对话选项，同时规范模型列表帮助与 `--all` 参数校验
...
- 添加天气、节假日、Web 与最终幻想 XIV 内置工具；`web_search` 通过 `-s` 按需启用，支持物价、时尚品鉴、FFLogs 角色排名及安全读取网页、文本、JSON 和 PDF
...
- 修复模型列表未按需显示全局配置与视觉能力、`zssm` 未自动选择本群视觉模型的问题
