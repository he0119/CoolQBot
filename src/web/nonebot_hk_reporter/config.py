...
class Config(metaclass=Singleton):
    ...
    def list_subscribe(self, user, user_type):
        query = Query()
        if user_sub := self.user_target.get((query.user == user)
    ...
    def del_subscribe(self, user, user_type, target, target_type):
        ...
        for idx, sub in enumerate(subs):
            if sub.get('target') == target and sub.get(
                    'target_type') == target_type:
                subs.pop(idx)
                self.user_target.update({'subs': subs}, query)
                self.update_send_cache()
    ...
    def update_send_cache(self):
        ...
        for user in self.user_target.all():
            for sub in user.get('subs', []):
                ...
                cat_res[sub['target_type']][sub['target']]['{}-{}'.format(
                    user['user_type'], user['user'])] = sub['cats']
                tag_res[sub['target_type']][sub['target']]['{}-{}'.format(
                    user['user_type'], user['user'])] = sub['tags']
    ...
    def get_sub_category(self, target_type, target, user_type, user):
        return self.target_user_cat_cache[target_type][target]['{}-{}'.format(
            user_type, user)]

    def get_sub_tags(self, target_type, target, user_type, user):
        return self.target_user_tag_cache[target_type][target]['{}-{}'.format(
            user_type, user)]
...
def start_up():
    ...
    if not (search_res := config.kv_config.search(Query().name == "version")):
        config.kv_config.insert({
            "name": "version",
            "value": config.migrate_version
        })
        ...
        config.kv_config.update({"value": config.migrate_version},
                                version_query)
        # do migration
