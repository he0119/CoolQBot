...
class Weibo(NewMessage, TargetMixin):
    ...
    async def get_target_name(self, target: Target) -> Optional[str]:
        async with httpx.AsyncClient() as client:
            ...
            res = await client.get('https://m.weibo.cn/api/container/getIndex',
                                   params=param)
    ...
    async def get_sub_list(self, target: Target) -> list[RawPost]:
        async with httpx.AsyncClient() as client:
            ...
            if not res_data['ok']:
                return []
            custom_filter: Callable[[RawPost],
                                    bool] = lambda d: d['card_type'] == 9
    ...
    def get_date(self, raw_post: RawPost) -> float:
        created_time = datetime.strptime(raw_post['mblog']['created_at'],
                                         '%a %b %d %H:%M:%S %z %Y')
        ...
        res = list(
            map(
                ...
                filter(
                    ...
                    map(lambda x: x.text,
                        soup.find_all('span', class_='surl-text')))))
        super_topic_img = soup.find(
            'img', src=re.compile(r'timeline_card_small_super_default'))
        if super_topic_img:
            try:
                res.append(
                    super_topic_img.parent.parent.find(
                        'span', class_='surl-text').text + '超话')
            except:
    ...
    def get_category(self, raw_post: RawPost) -> Category:
        if raw_post['mblog'].get('retweeted_status'):
            return Category(1)
        elif raw_post['mblog'].get('page_info') and raw_post['mblog'][
    ...
    async def parse(self, raw_post: RawPost) -> Post:
        ...
        if info['isLongText'] or info['pic_num'] > 9:
            async with httpx.AsyncClient() as client:
                res = await client.get('https://m.weibo.cn/detail/{}'.format(
                    info['mid']),
                                       headers=header)
            try:
                full_json_text = re.search(
                    r'"status": ([\s\S]+),\s+"hotScheme"', res.text).group(1)
                info = json.loads(full_json_text)
            except:
                logger.info(
                    'detail message error: https://m.weibo.cn/detail/{}'.
                    format(info['mid']))
        parsed_text = self._get_text(info['text'])
        pic_urls = [img['large']['url'] for img in info.get('pics', [])]
        detail_url = 'https://weibo.com/{}/{}'.format(info['user']['id'],
                                                      info['bid'])
