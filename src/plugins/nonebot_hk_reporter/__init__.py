...
def do_add_sub(add_sub: Type[Matcher]):
    ...
    async def parser_cats(bot: Bot, event: Event, state: T_State):
        ...
        for cat in str(event.get_message()).strip().split():
            ...
            res.append(
                platform_manager[state['platform']].reverse_category[cat])
        state['cats'] = res
...
add_sub_matcher = on_command("添加订阅", block=True)
do_add_sub(add_sub_matcher)
manage_add_sub_mather = on_command('管理-添加订阅', block=True)
...
query_sub_macher = on_command("查询订阅", block=True)
do_query_sub(query_sub_macher)
manage_query_sub_mather = on_command('管理-查询订阅', block=True)
...
del_sub_macher = on_command("删除订阅", block=True)
do_del_sub(del_sub_macher)
manage_del_sub_mather = on_command('管理-删除订阅', block=True)
