...
class Config(BaseSettings):
    ...
    push_news_interval: int = int(
        DATA.config.get('news', 'push_news_interval', '30'))
    # 上次推送新闻的发布 ID
    ...
    calender_minute: int = int(
        DATA.config.get('calender', 'minute', fallback='30'))
    calender_second: int = int(
        DATA.config.get('calender', 'second', fallback='0'))
