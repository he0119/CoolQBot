...
| `/agent <内容>`                       | -                 | 使用全部工具完成问答   | 所有人     |
| `/agent --model <模型名> <内容>`      | -                 | 指定模型进行工具问答   | 所有人     |
...
对话及解释选项应写在内容前；`<内容>` 是可变长参数，放在内容后的选项可能被当作正文。`/chat` 与 `/agent` 都支持 `--model`、`-c`、`-r` 和 `-t`；@ 对话支持相同选项，并可用 `-a|--agent` 按次启用工具增强模式。
...
/agent 查询 Python 3.14 新特性   # 工具增强模式默认可以搜索网页
/agent 查一下静语庄园的萨维奈舞裙价格 # 模型可直接调用 FF14 物价工具
...
`--model` 只覆盖本次解释，不会修改群组设置。未通过 `/llm model set-zssm` 单独设置解释模型时，`zssm` 跟随本群默认模型；`/llm model clear-zssm` 可恢复这一行为。解释模式使用独立提示词，不展示推理过程，也不开放工具调用；即使模型返回了工具调用请求也会被拒绝。机器人的回答引用你发送的 `zssm` 命令。
...
三个最终幻想 XIV 工具随 `/agent` 提供：物价查询默认使用猫小胖大区，也可以显式指定国服服务器或大区；
