...
`/llm model list` 对所有人都只显示本群已启用的模型及其“默认对话”、`zssm` 和 `zssm 视觉`用途标签，并分区列出单次对话与群组设置命令；超级管理员增加 `--all` 后才会看到全局全部模型及本群启用状态。增加 `--capabilities` 后，当前列表才会同时显示每个模型的能力，两个参数可以组合使用。
...
超级管理员使用 `/llm model enable <模型名...>` 为本群增量启用一个或多个模型，使用 `/llm model disable <模型名...>` 增量禁用；两个命令都可增加 `--all` 操作全部模型。未设置过的群默认不启用任何模型。默认模型、临时 `--model`、额度查询以及 `zssm` 的解释和视觉模型都不能越过该列表。禁用正在使用的模型时，默认模型会切换到剩余列表第一项，解释和显式视觉模型设置会被清除，视觉模型随后从剩余列表中重新自动选择。
