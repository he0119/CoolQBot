...
| `/llm model -l -a`                       | -                 | 查看全部模型及启用状态 | 超级管理员 |
| `/llm model -l -c`                       | -                 | 查看模型用途及能力     | 所有人     |
...
| `/llm model --clear-vision`              | -                 | 视觉模型恢复自动选择   | 群管理员   |
...
`/llm model --list` 对所有人都只显示本群已开放的模型及其“当前”、`zssm` 和 `zssm 视觉`用途标签；超级管理员增加 `-a` 后才会看到全局全部模型及本群启用状态。增加 `-c`（`--capabilities`）后，当前列表才会同时显示每个模型的能力，两个参数可以组合使用。
...
图片处理取决于模型能力：解释模型在 `capabilities` 中声明 `vision` 时，图片直接随解释请求发送，只调用一次；否则优先使用 `/llm model --set-vision` 显式指定的模型，未指定时按全局模型配置顺序自动选择本群首个已开放且声明了 `vision` 的模型，先生成图片描述再解释。`--clear-vision` 会清除显式选择并恢复自动选择；没有符合条件的模型时会给出明确提示。

超级管理员使用 `/llm model --set-available <模型名...>` 精确设置本群开放的模型；未设置过的群默认不允许使用任何模型，`--clear-available` 可清空本群的准入列表。默认模型、临时 `--model`、额度查询以及 `zssm` 的解释和视觉模型都不能越过该列表。修改开放列表时，已不再可用的默认模型会切换到列表第一项，解释和显式视觉模型设置会被清除，视觉模型随后从新列表中重新自动选择。
