...
对话及解释选项应写在内容前；`<内容>` 是可变长参数，放在内容后的选项可能被当作正文。`/chat` 支持 `--model`、`-c`、`-r`、`-t` 和 `-a|--agent`；`/agent` 是预先启用 `-a` 的快捷入口并支持其余相同选项。@ 对话与 `zssm` 也可用 `-a|--agent` 按次启用工具增强模式。
...
`--model` 只覆盖本次解释，不会修改群组设置。未通过 `/llm model set-zssm` 单独设置解释模型时，`zssm` 跟随本群默认模型；`/llm model clear-zssm` 可恢复这一行为。解释模式使用独立提示词且不展示推理过程；默认不开放工具，显式增加 `-a|--agent` 后会为本次解释开放包括 `web_search` 在内的全部工具。机器人的回答引用你发送的 `zssm` 命令。
