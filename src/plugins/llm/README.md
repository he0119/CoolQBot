...
解释模式使用独立提示词，不展示推理过程，也不开放工具调用。模型在 `capabilities` 中声明 `vision` 时，图片会直接随解释请求发送，只调用一次；否则需要通过 `LLM__ZSSM_VISION_MODEL` 指定另一个声明了 `vision` 的模型，先生成图片描述再解释。消息中实际出现的网页或 PDF 链接会在调用模型前读取；读取器拒绝本机、内网、带凭据和非 HTTP(S) 地址，并限制资源数量、下载大小、PDF 页数与提取文本长度。图片读取、外部资源读取以及视觉/解释模型调用会输出简洁日志；资源日志只记录主机名和处理结果，不记录 URL 路径、查询参数或正文。
